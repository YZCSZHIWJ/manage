            ...
            <div class="col-xs-12">
              <section class="panel">
                ...
                <div class="panel-body">
                  <form class="form-inline" action="/order/canceledflowlist" method="GET">
                  ...
                  <section>
                    <table class="table table-bordered table-striped table-condensed orderlist">
                      ...
                      <#if (page?? && page.elements?size>0)>
                        ...
                        <tr>
                          <td><a href="/order/canceledflowlist/detail?id=${item.id}" target="_blank">${item.id}</a></td>
                    ...
                    <script type="text/javascript">
                      var pageStr = createPageBar2(${page.pageNumber}, ${page.pageSize}, ${page.pageIndex}, ${page.total}, 'goPager', 5, "/order/canceledflowlist?<#if id??>id=${id}&</#if><#if tid??>tid=${tid}&</#if><#if userid??>userid=${userid}&</#if><#if sellerid??>sellerid=${sellerid}&</#if><#if plat??>plat=${plat}&</#if><#if device??>device=${device}&</#if><#if status??>status=${status}&</#if><#if account??>account=${account}&</#if>");
